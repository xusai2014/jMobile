...
import {loginHelper,nativeRequestBaseParams} from "../../interface/jsNative";
  ...
  render() {
    ...
    return [<div style={{background: '#FFFFFF', paddingBottom: "0.7rem"}}>
      <div style={styles.top}>
        <div style={styles.topText}>7日内待还<span style={styles.topSubText}>{isLogged ? '10' : '0'}笔</span>
        </div>
        <img onClick={() => {
          this.loginEnter(1)
      ...
      <div style={{...styles.cover}}>
        {
          [{
            ...
            type: "0"
          }, {img: "/static/img/banka@2x.png", text: "办卡", action: "https://www.baidu.com", type: '1'}].map((v, k) => {
      ...
      <div style={{display: "flex", justifyContent: "space-around", marginTop: "0.49rem"}}>{
        isLogged?[{
          ...
          return <div style={{display: "inline-block"}}><img style={{width: '0.74rem'}} src={imgSrc}/>
            <div>{title}</div>
          </div>
        }):null
      }</div>
      ...
      {
        isLogged?[1,2].map(()=><BillCard repay={() => this.setState({visible: true})}/>):[1].map(() => {
          return <BillCard repay={() => this.loginEnter(5)}/>
        })
      }
    </div>,
      <div style={{
